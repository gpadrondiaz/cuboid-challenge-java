...
public class CuboidController {
    ...
    @PutMapping("/{cuboidId}")
    public ResponseEntity<CuboidDTO> update(@PathVariable (value = "cuboidId") final Long cuboidId, @Valid @RequestBody final CuboidDTO cuboidDTO) {
        CuboidDTO cuboid = service.update(cuboidId, cuboidDTO);
        return new ResponseEntity<>(cuboid, HttpStatus.OK);
    }

    @DeleteMapping("/{cuboidId}")
    public ResponseEntity<CuboidDTO> delete(@PathVariable (value = "cuboidId") final Long cuboidId) {
