...
class CuboidControllerTest {
    ...
    void shouldUpdateCuboid() throws Exception {
        CuboidDTO cuboidDTO = CuboidDTO.builder().id(1l)
                .width(2f).height(2f).depth(2f).volume(8d).bagId(3L).build();

        this.mockMvc.perform(put(PATH + "/{cuboidId}", 1l).contentType(MediaType.APPLICATION_JSON).param("cuboidId", "1l")
    ...
    void shouldDeleteCuboid() throws Exception {
        this.mockMvc.perform(delete(PATH + "/{cuboidId}", 1l))
                .andExpect(status().isOk());
    }

    @Test
    void shouldNotDeleteWhenCuboidNotFound() throws Exception {
        this.mockMvc.perform(delete(PATH + "/{cuboidId}", 100l))
                .andExpect(status().isOk());;
    }
